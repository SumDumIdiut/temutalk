...
| `public/assistant/` | Voice assistant UI: wake-word listening starts automatically on page load (no toggle/button), speech-to-text via Web Speech API or whisper.cpp fallback, spoken reply played back from `/api/assistant/tts` through a plain `<audio>` element, executes device actions (play_radio, set_timer, navigate) via tab-script globals. Visual feedback: an orange edge-glow while awake/processing, plus a popup showing the exchange with each reply word highlighted in sync as it's spoken |
