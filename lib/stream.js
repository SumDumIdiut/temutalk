...
<body>
  ...
  <div class="info">
    Stream URL: <span class="stream-link">${BASE_PATH}/stream</span><br>
  ...
  <script>
    ...
    function startPlayer(){if(playerRunning)return;playerRunning=true;player.src=BASE_PATH+'/stream?t='+Date.now();player.style.display='none';volRow.style.display='block';player.play().catch(()=>{});}
    function stopPlayer(){playerRunning=false;player.pause();player.src='';volRow.style.display='none';}
    function reconnect(){if(!playerRunning)return;player.pause();player.src=BASE_PATH+'/stream?t='+Date.now();player.load();player.play().catch(()=>{});}
    ...
    async function checkStatus(){try{const res=await fetch(BASE_PATH+'/api/broadcast/status');if(!res.ok)throw new Error('HTTP '+res.status);const r=await res.json();setUI(r.running,r.running?'Live - streaming via ffmpeg (PID '+r.pid+')':'Idle - not streaming');}catch(e){dot.className='dot warn';stxt.textContent='Server error: '+e.message;}}
    btnStart.onclick=async()=>{btnStart.disabled=true;dot.className='dot warn';stxt.textContent='Starting ffmpeg...';try{const res=await fetch(BASE_PATH+'/api/broadcast/start',{method:'POST'});const r=await res.json();if(r.ok)setTimeout(checkStatus,1500);else{stxt.textContent='Failed: '+(r.error||'unknown');btnStart.disabled=false;}}catch(e){stxt.textContent='Error: '+e.message;btnStart.disabled=false;}};
    btnStop.onclick=async()=>{btnStop.disabled=true;await fetch(BASE_PATH+'/api/broadcast/stop',{method:'POST'}).catch(()=>{});setTimeout(checkStatus,500);};
    checkStatus();
