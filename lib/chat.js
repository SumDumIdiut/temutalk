...
(function chatLoad() {
  try {
    ...
    // chatSaveSync (not the debounced chatSave) because this all runs once
    // during startup: without an explicit, immediate write here, whatever
    // the two migrations above just changed in memory would only reach disk
    // whenever some unrelated later chat action happened to trigger a save,
    // leaving the on-disk file (mis)reporting pre-migration data in the
    // meantime (or, worse, never, if the server never restarts and nothing
    // else ever calls chatSave first). Harmless to always run even when
    // neither migration found anything to change.
    chatSaveSync();
