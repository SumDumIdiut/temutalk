...
// In-memory station search for the voice assistant (lib/assistant.js).
// Matches every query word against name+country+tags, ranks exact/prefix
// name matches first, then by votes.
function searchRadio(query, limit = 5) {
  const q = String(query || '').toLowerCase().trim();
  if (!q) return [];
  const words = q.split(/\s+/);
  const scored = [];
  for (const s of radioCache) {
    const name = (s.name || '').toLowerCase();
    const hay  = name + ' ' + (s.country || '').toLowerCase() + ' ' + (s.tags || '').toLowerCase();
    if (!words.every(w => hay.includes(w))) continue;
    let score = s.votes || 0;
    if (name === q) score += 1e9;
    else if (name.startsWith(q)) score += 1e6;
    else if (name.includes(q)) score += 1e3;
    scored.push([score, s]);
  }
...
function radioCacheReady() { return radioCache.length > 0; }

// ─── News (BBC RSS, cached — shared by /api/news and the voice assistant) ───
const NEWS_FEEDS = {
...
const newsCache  = new Map(); // cat → { ts, items }
...
async function fetchNews(cat = 'top', limit = 30) {
  cat = NEWS_FEEDS[cat] ? cat : 'top';
  const cached = newsCache.get(cat);
  if (cached && Date.now() - cached.ts < NEWS_TTL) return cached.items;
  const r = await axios.get(NEWS_FEEDS[cat], { headers: { 'User-Agent': 'TemuTalk/1.0' }, timeout: 10000 });
  const items = [...String(r.data).matchAll(/<item>([\s\S]*?)<\/item>/g)].map(m => {
    const b = m[1];
    return {
      title:       xmlDecode(xmlTag(b, 'title')),
      link:        xmlDecode(xmlTag(b, 'link')),
      description: xmlDecode(xmlTag(b, 'description')),
      pubDate:     xmlTag(b, 'pubDate'),
      thumbnail:   (b.match(/<media:thumbnail[^>]*url="([^"]+)"/) || [])[1] || null,
      source:      'BBC News',
    };
  }).filter(i => i.title).slice(0, limit);
  newsCache.set(cat, { ts: Date.now(), items });
  return items;
}

// ─── Weather fetch (shared by /api/weather and the voice assistant) ──────────
// Accepts a city name or "lat,long" — wttr.in resolves coordinates to the
// nearest town itself (slow on first resolution, hence the generous timeout).
async function fetchWeather(query) {
  const q = String(query || '').trim();
  const r = await axios.get(`https://wttr.in/${encodeURIComponent(q)}?format=j1`,
    { headers: { 'User-Agent': 'GoogleHomeHub/1.0' }, timeout: 25000 });
  return r.data;
}
...
module.exports = function setupDataRoutes(app, WEATHER_CITY) {
  ...
  app.get('/api/radio/proxy', (req, res) => {
    ...
    // CONNECT_TIMEOUT_MS covers establishing the connection and receiving
    // response headers -- reasonable to keep tight. STREAM_IDLE_TIMEOUT_MS
    // is what actually matters for playback: a live internet radio stream
    // (this proxy's only real use -- notably low-bitrate/overseas stations
    // like state broadcasters) routinely has multi-second gaps between
    // chunks even while perfectly healthy. The old code used 10s for both
    // phases, so any such gap killed the connection out from under
    // otherwise-fine playback -- exactly the "random audio drops" symptom
    // this exists to avoid. One 'timeout' listener handles both phases
    // (checking res.headersSent to tell them apart) -- calling
    // request.setTimeout() a second time with its own callback would add a
    // second listener rather than replace the first, firing both.
    const CONNECT_TIMEOUT_MS = 10000;
    const STREAM_IDLE_TIMEOUT_MS = 45000;
    const upstream = mod.get(url, { headers: { 'User-Agent': 'TemuTalk/1.0', 'Icy-MetaData': '0' } }, up => {
      ...
      // Streaming has actually started -- re-arm the same idle timer with
      // a much longer allowance now that brief gaps are expected/normal.
      upstream.setTimeout(STREAM_IDLE_TIMEOUT_MS);
      up.pipe(res);
  ...
  // lrclib.net's /api/get is an exact-match lookup, not a search. album is
  // the least reliable field to match on -- Spotify's raw album title
  // routinely differs from lrclib's stored one (deluxe/remaster suffixes,
  // soundtrack naming, regional editions), and confirmed live: a
  // mismatched album_name causes a hard 404 even when the same
  // artist+track alone would have succeeded. duration is a much safer
  // bet -- lrclib treats it as a "closest version" hint rather than a
  // strict filter (confirmed live: even a duration 50s off from the real
  // value still matched a real record, just picking the closest one),
  // so it never causes a spurious miss on its own.
  //
  // Try progressively looser /api/get lookups instead of one shot, then
  // fall back to /api/search (genuinely fuzzy) if even a bare
  // artist+track lookup 404s -- e.g. when track_name itself has extra
  // formatting ("- Remaster 2011") that lrclib's canonical entry lacks.
  async function lrclibGet(params) {
    try {
      ...
      throw e; // network/timeout/5xx -- not worth retrying with a looser query
    }
  }
  ...
  app.get('/api/lyrics', async (req, res) => {
    ...
    const duration = parseInt(req.query.duration, 10) || null;
    if (!artist || !track) return res.status(400).json({ error: 'artist and track required' });
    try {
      let d = null;
      if (album) d = await lrclibGet({ artist_name: artist, track_name: track, album_name: album, ...(duration ? { duration } : {}) });
      if (!d && duration) d = await lrclibGet({ artist_name: artist, track_name: track, duration });
      if (!d) d = await lrclibGet({ artist_name: artist, track_name: track });
      if (!d) {
        const s = await axios.get('https://lrclib.net/api/search', {
          params: { artist_name: artist, track_name: track }, timeout: 8000,
        });
        const results = s.data || [];
        d = duration
          ? results.reduce((best, r) => (!best || Math.abs(r.duration - duration) < Math.abs(best.duration - duration)) ? r : best, null)
          : (results[0] || null);
      }
  ...
  app.get('/api/crypto', async (req, res) => {
    ...
    try {
      const r = await axios.get('https://api.coingecko.com/api/v3/coins/markets', {
        params: { vs_currency: 'usd', ids, order: 'market_cap_desc', per_page: 20, page: 1, sparkline: true, price_change_percentage: '24h' },
  ...
  app.get('/api/rates/history', async (req, res) => {
    const base = (req.query.base || 'USD').toUpperCase().replace(/[^A-Z]/g, '').slice(0, 3);
    const to   = (req.query.to   || 'EUR').toUpperCase().replace(/[^A-Z,]/g, '').slice(0, 100);
  ...
  app.get('/api/news', async (req, res) => {
    try {
      res.json(await fetchNews(String(req.query.cat || 'top').replace(/[^a-z]/g, '')));
    } catch (e) { res.status(502).json({ error: e.message }); }
  });

  // 7-day forecast by coordinates (Open-Meteo; wttr.in only gives 3 days)
  app.get('/api/forecast', async (req, res) => {
    ...
    try {
      ...
      res.json(r.data.daily || {});
    } catch (e) { res.status(502).json({ error: e.message }); }
  });
};

module.exports.searchRadio     = searchRadio;
module.exports.radioCacheReady = radioCacheReady;
