...
module.exports = function setupDataRoutes(app, WEATHER_CITY) {
  ...
  app.get('/api/stock', async (req, res) => {
    ...
    const range = ['1d','5d','1mo','3mo','6mo','1y','5y'].includes(req.query.range) ? req.query.range : '1mo';
    const ivMap = { '1d':'5m', '5d':'15m', '1mo':'1d', '3mo':'1d', '6mo':'1wk', '1y':'1wk', '5y':'1mo' };
    try {
  ...
  app.get('/api/iss', async (req, res) => {
    try {
      const [issR, cssR] = await Promise.allSettled([
        axios.get('https://api.wheretheiss.at/v1/satellites/25544', { timeout: 8000 }),
        axios.get('https://api.wheretheiss.at/v1/satellites/48274', { timeout: 8000 }),
      ]);
      res.json({
        iss: issR.status === 'fulfilled' ? issR.value.data : null,
        css: cssR.status === 'fulfilled' ? cssR.value.data : null,
      });
    } catch (e) { res.status(502).json({ error: e.message }); }
  });
