...
module.exports = function setupWebSocket(wss, broadcastLiveList, ASSET_VERSION) {
  wss.on('connection', (ws, req) => {
    ...
    ws.on('message', (raw, isBinary) => {
      ...
      // file plus lib/chat.js/lib/broadcast.js, and instrumenting every one
      // individually wasn't worth the invasiveness. This still covers every
      // client-initiated action, which is what "latency" mostly means here.
      ...
      if (msg.type === 'join') {
        ...
        chat.sendMyServers(ws, wsDeviceId);
        return;
      }
      ...
      if (msg.type === 'radio-remote-cmd' && msg.targetDeviceId && msg.action) {
        ...
        if (targets?.size) {
          const out = JSON.stringify({
            ...
            fromName: wsDeviceId ? chat.chatGetName(wsDeviceId) : '',
          });
