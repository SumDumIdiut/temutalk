...
// Similar-track lookup for autoplay + magic shuffle, with a fallback chain
// instead of a hard dependency on /v1/recommendations -- Spotify deprecated
// that endpoint (along with /audio-features, /audio-analysis) for API apps
// created after Nov 2024, and this app runs on a per-user Client ID/Secret
// entered through the UI (see /api/save-creds), so there's no way to know
// in advance whether a given user's app still has access to it. Each stage
// only runs if earlier stages haven't filled `limit` yet, and a failure
// anywhere (403, deprecated, network) just falls through to the next stage
// rather than failing the whole request.
async function fetchSimilarTracks(token, { seedTrackId, seedArtistId, limit, excludeIds }) {
  const exclude = new Set(excludeIds);
  const seen    = new Set();
  const results = [];
  const addTrack = t => {
    if (!t?.id || !t.uri || exclude.has(t.id) || seen.has(t.id)) return;
    seen.add(t.id);
    results.push({ id: t.id, uri: t.uri, name: t.name, duration_ms: t.duration_ms, artists: t.artists, album: t.album });
  };
  const headers = { Authorization: `Bearer ${token}` };

  // 1) Spotify's own recommendation engine -- best results when available.
  if (seedTrackId && results.length < limit) {
    try {
      const params = new URLSearchParams({ limit: String(limit * 2), market: 'from_token', seed_tracks: seedTrackId });
      if (seedArtistId) params.set('seed_artists', seedArtistId);
      const r = await axios.get(`https://api.spotify.com/v1/recommendations?${params}`, { headers });
      (r.data.tracks || []).forEach(addTrack);
    } catch (_) {}
  }

  // 2) More tracks from the same artist.
  if (seedArtistId && results.length < limit) {
    try {
      const r = await axios.get(`https://api.spotify.com/v1/artists/${seedArtistId}/top-tracks?market=from_token`, { headers });
      (r.data.tracks || []).forEach(addTrack);
    } catch (_) {}
  }

  // 3) Same genre via search -- genres come off the artist object itself
  //    (unlike /recommendations, Spotify hasn't deprecated that).
  if (seedArtistId && results.length < limit) {
    try {
      const ar = await axios.get(`https://api.spotify.com/v1/artists/${seedArtistId}`, { headers });
      const genre = ar.data.genres?.[0];
      if (genre) {
        const r = await axios.get(`https://api.spotify.com/v1/search?q=${encodeURIComponent('genre:"' + genre + '"')}&type=track&limit=${limit * 2}`, { headers });
        (r.data.tracks?.items || []).forEach(addTrack);
      }
    } catch (_) {}
  }

  // 4) Always-available fallback: the user's own top tracks. Not "similar"
  //    in a strict sense, but guaranteed to work off core, non-deprecated
  //    endpoints already used elsewhere in this app, so autoplay/magic
  //    shuffle degrade to "something you actually listen to" worst case,
  //    never a hard failure.
  if (results.length < limit) {
    try {
      const r = await axios.get('https://api.spotify.com/v1/me/top/tracks?limit=50&time_range=medium_term', { headers });
      (r.data.items || []).forEach(addTrack);
    } catch (_) {}
  }
...
// Every connected device polled in parallel -- this used to await each
// device's Spotify round-trip inside a sequential for-of loop, so with
// several devices connected the last one in iteration order was waiting on
// every earlier device's full request before its own even started (visible
// in telemetry as outbound /v1/me/player calls staggering out one after
// another every 3s instead of clustering together).
async function pushOneDeviceState(deviceId) {
  if (!state.deviceClients.get(deviceId)?.size) return;
  const token = await getDeviceToken(deviceId);
  if (!token) { broadcastToDevice(deviceId, { type: 'player', authenticated: false }); return; }
  try {
...
async function pushAllDeviceStates() {
  await Promise.allSettled([...state.deviceClients.keys()].map(pushOneDeviceState));
}
// Was 3000 -- the client-side ticker (onPlayer() in music/script.js)
// correctly freezes the playhead the instant a poll reports
// is_playing:false, verified directly, so the "pause on my phone doesn't
// pause the playhead here" lag is entirely explained by how long it takes
// for that fresh state to actually reach the client: this interval, plus
// whatever Spotify's own Connect-state propagation adds on top (a real,
// external delay -- can't be shortened from here). Tightening the one
// half of that we do control still directly shrinks the worst case.
setInterval(pushAllDeviceStates, 1500);
...
module.exports = function setupSpotifyRoutes(app, REDIRECT_URI, MAIN_BASE) {
  ...
  // Registered before the generic /api/player/:action route below --
  // Express matches routes in registration order, and ":action" matches the
  // literal string "seek" too, so this was silently shadowing the seek
  // route entirely: every seek request hit the wildcard handler first,
  // found "seek" missing from its action map, and returned 400 "Unknown
  // action" before the real handler ever ran. This was the actual cause of
  // "clicking a lyric line highlights it but never moves the song" --
  // nothing about local playback state, just an unreachable route.
  ...
  // Spotify has no POST /v1/me/playlists shortcut -- creating one is only
  // exposed via /v1/users/{user_id}/playlists, so the current user's own id
  ...
  // Spotify has no true "delete playlist" endpoint for a user's own library
  // -- unfollowing is the documented way to remove one, same end result the
  // client cares about (it disappears from /api/playlists).
  app.delete('/api/playlist/:id', async (req, res) => {
    ...
    if (!token) return res.status(401).json({ error: 'Not authenticated' });
    try {
      await axios.delete(`https://api.spotify.com/v1/playlists/${req.params.id}/followers`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      res.json({ ok: true });
    } catch (e) { res.status(e.response?.status || 500).json({ error: e.response?.data?.error?.message || e.message }); }
  ...
  // Powers both autoplay (limit=1, seeded from whatever just finished) and
  // magic shuffle (limit=N, seeded per-track while building a mixed queue).
  app.get('/api/similar-tracks', async (req, res) => {
    const deviceId = resolveDevice(req);
    if (!deviceId) return res.status(400).json({ error: 'device id required' });
    const token = await getDeviceToken(deviceId);
    if (!token) return res.json({ authenticated: false });
    const seedTrackId  = (req.query.seedTrackId  || '').trim() || null;
    const seedArtistId = (req.query.seedArtistId || '').trim() || null;
    if (!seedTrackId && !seedArtistId) return res.status(400).json({ error: 'seedTrackId or seedArtistId required' });
    const limit      = Math.min(Math.max(parseInt(req.query.limit) || 1, 1), 20);
    const excludeIds = (req.query.exclude || '').split(',').map(s => s.trim()).filter(Boolean);
    try {
      const tracks = await fetchSimilarTracks(token, { seedTrackId, seedArtistId, limit, excludeIds });
      res.json({ tracks });
    } catch (e) { res.status(500).json({ error: e.response?.data?.error?.message || e.message }); }
  });
