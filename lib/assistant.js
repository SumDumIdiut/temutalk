...
// finance, news, timers).
//
// Server tools execute here; device tools (play radio, set timer, navigate)
// are queued into `actions` and returned to the browser, which executes them
// with the existing tab-script globals (playStation, addTimer…).
//
...
const SYSTEM_PROMPT = `You are the voice assistant built into TemuTalk, a smart home hub with tabs for music (Spotify), live radio, weather, finance (stocks/crypto/forex), news (Hacker News), and timers.
...
- Device tools (radio, timers, navigation) are queued and run on the user's device the moment you finish - treat a "queued" tool result as success.
...
const TOOLS = [
  ...
  },
  {
    ...
    input_schema: { type: 'object', properties: {
      tab: { type: 'string', enum: ['home', 'music', 'radio', 'finance', 'news', 'weather', 'timer', 'system'] },
    }, required: ['tab'] },
  },
];
...
// Spotify's search sometimes ranks a "Greatest Hits"/decade compilation
// reissue above the track's original studio album (same recording, cover
// reads as a curated playlist instead of "the song"). Only swap to a
// non-compilation release of the SAME song (matched on name + artist) —
// picking the first non-compilation result from the whole candidate pool
// previously grabbed an unrelated track whenever the correct match itself
// happened to be compilation-tagged (e.g. a "Bohemian Rhapsody" remaster
// swapped for an unrelated "Karaoke Girl" cover). Trust Spotify's own top
// relevance match unless a clearly-identical alternative is right there.
function pickBestTrackMatch(tracks) {
  const top = tracks[0];
  if (!top || top.albumType !== 'compilation') return top;
  const sameSong = tracks.find(t =>
    t.albumType !== 'compilation' &&
    t.name.toLowerCase() === top.name.toLowerCase() &&
    t.artists.toLowerCase() === top.artists.toLowerCase()
  );
  return sameSong || top;
}
...
function setupAssistantRoutes(app, resolveDevice, weatherCity) {
  ...
  app.post('/api/assistant/tts', async (req, res) => {
    ...
    try {
      const wav = await tts.synthesize(text, req.body?.rate);
      console.log('[assistant] tts:', JSON.stringify(text.slice(0, 60)), '→', wav.length, 'bytes');
      res.set('Content-Type', 'audio/wav');
      res.send(wav);
    } catch (e) {
      console.error('[assistant] tts failed:', e.message || e);
      res.status(503).json({ error: e.message || 'Text-to-speech failed.' });
    }
  });
