...
async function toolPlayPlaylist(input, deviceId) {
  ...
  try {
    await transferAndPlay(token, { context_uri: match.uri }, null);
  } catch (e) {
    if (e.response?.status !== 404) throw new Error(e.response?.data?.error?.message || e.message);
    ...
    await transferAndPlay(token, { context_uri: match.uri }, spDeviceId);
  }
...
async function toolAddToPlaylist(input, deviceId) {
  ...
  if (input.track_query && input.track_query.trim()) {
    const { tracks } = await spotifySearchTracks(deviceId, input.track_query, 1);
    if (!tracks.length) throw new Error(`No Spotify results for "${input.track_query}"`);
    ({ uri: trackUri, name: trackName, artists: trackArtists } = tracks[0]);
  } else {
    const player = state.playerStateCache.get(deviceId);
...
async function toolPlayMusic(input, deviceId) {
  const { token, tracks } = await spotifySearchTracks(deviceId, input.query, 1);
  if (!tracks.length) throw new Error('No Spotify results for "' + input.query + '"');
  const t = tracks[0];
  try {
    await transferAndPlay(token, { uris: [t.uri] }, null);
  } catch (e) {
    if (e.response?.status !== 404) throw new Error(e.response?.data?.error?.message || e.message);
    const spDeviceId = await activateDevice(token);
    if (!spDeviceId) throw new Error('No Spotify playback device available. Open Spotify on a device first.');
    await transferAndPlay(token, { uris: [t.uri] }, spDeviceId);
  }
